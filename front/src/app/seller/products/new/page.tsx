...
export default function ProductCreatePage() {
    ...
    const subCategoryMap: Record<number, { id: number; name: string }[]> = {
        1: [
            { id: 11, name: '소파' },
            { id: 12, name: '침대' },
            { id: 13, name: '식탁/의자' },
    ...
    // 배송정책 (A안)
    const [deliveryType, setDeliveryType] = useState('무료배송');
    const [deliveryCost, setDeliveryCost] = useState('');
    const [regionLimit, setRegionLimit] = useState('');

    // 상태
    const [status, setStatus] = useState('상'); // 상, 중, 하
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 필수값 검증
        if (!name || !price || !stock || !description || !width || !depth || !height || !categoryId) {
        ...
        if (deliveryType === '유료배송' && !deliveryCost) {
            alert('배송비를 입력해주세요.');
            return;
        }
        ...
        console.log('배송 유형 값:', deliveryType); // 이 줄 추가
        formData.append('deliveryPolicy.type', deliveryType);
        formData.append('deliveryPolicy.cost', String(Number(deliveryCost || 0)));
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div style={{ marginBottom: '1rem' }}>
                            ...
                            <select
                                value={mainCategory}
                                onChange={(e) => {
                                    setMainCategory(Number(e.target.value));
                                    setCategoryId(''); // 대분류 바꾸면 소분류 초기화
                                }}
                                style={{ width: '100%', padding: '0.5rem', marginTop: '0.25rem' }}
                            >
