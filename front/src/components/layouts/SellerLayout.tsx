      ...
      <div className="flex flex-col lg:flex-row flex-1 w-full h-full">
        ...
        <div className="lg:hidden">
          <SellerSidebar 
            ...
            className={`fixed z-50 top-0 left-0 h-full w-60 bg-gray-800 text-white flex-shrink-0 transition-transform duration-300 ease-in-out ${
              sidebarOpen ? 'translate-x-0' : '-translate-x-full'
            }`} 
          />
        </div>
        {/* 데스크탑 사이드바: 항상 렌더 */}
        <div className="hidden lg:block h-full w-60 bg-gray-800 text-white flex-shrink-0">
          <SellerSidebar className="h-full w-60 bg-gray-800 text-white flex-shrink-0" />
        </div>
        {/* 본문: flex-1로 꽉 차게 */}
        <main className="flex-1 w-full min-h-0 px-4 py-8 bg-gray-100">
          {children}
        </main>
      </div>
    </div>
