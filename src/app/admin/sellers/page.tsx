      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="block md:hidden space-y-4">
          {filteredSellers.map((seller, idx) => (
            <div key={seller.id} className="bg-white rounded shadow p-4">
              <div className="font-bold mb-2">번호: {idx + 1}</div>
              <div className="mb-1">이름: {seller.name}</div>
              <div className="mb-1">이메일: {seller.email}</div>
              <div className="mb-1">
                활성 상태:
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {!seller.is_approved && (
                  <>
                    ...
                    <button
                      className="px-3 py-1 rounded text-white bg-orange-500"
                      onClick={() => handleApproval(seller, false)}
                    >
                      거절
                    </button>
                ...
                <button
                  className={`px-3 py-1 rounded text-white ${seller.is_active ? 'bg-red-600' : 'bg-green-600'}`}
                  onClick={() => handleToggleActive(seller)}
                >
                  {seller.is_active ? '정지' : '복구'}
                </button>
